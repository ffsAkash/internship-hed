...
!(function (a, b) {
 ...
 function L(a, b, c) {
    if (b.enablePagination) {
        ...
        // ADD {2} for custom classes
        e = '<li><a href="#{0}" class="{2}" role="menuitem">{1}</a></li>',
        f = "";

        // Previous button
        if (!b.forceMoveForward)
            f += e.format("previous", b.labels.previous, "btn-prev myPrevClass");

        // Next button
        f += e.format("next", b.labels.next, "btn-next myNextClass");

        // Finish button
        if (b.enableFinishButton)
            f += e.format("finish", b.labels.finish, "btn-finish myFinishClass");

        // Cancel button
        if (b.enableCancelButton)
            f += e.format("cancel", b.labels.cancel, "btn-cancel myCancelClass");
